...
public class CarDisplayInputLockTest {
    ...
    @Rule @ClassRule public static final DeviceState sDeviceState = new DeviceState();
    ...
    public void setUp() throws Exception {
        ...
        assumeTrue(
                "This test is enabled only in multi-user/multi-display devices",
    ...
    private void assertDisplayInputSinkCreated(int displayId, boolean created) {
        PollingCheck.waitFor(
                INPUT_LOCK_UPDATE_WAIT_TIME_MS,
                () -> {
    ...
    private String getDisplayInputLockSetting(@NonNull ContentResolver resolver) {
        return Settings.Global.getString(resolver, CarSettings.Global.DISPLAY_INPUT_LOCK);
    }
    ...
    private void writeDisplayInputLockSetting(
            @NonNull ContentResolver resolver, @NonNull String value) {
    ...
    private void mouseClickOnDisplay(int displayId) {
        injectMotionEvent(obtainMouseEvent(mActivity.mView, MotionEvent.ACTION_DOWN, displayId));
        injectMotionEvent(
                obtainMouseEvent(mActivity.mView, MotionEvent.ACTION_BUTTON_PRESS, displayId));
        injectMotionEvent(
                obtainMouseEvent(mActivity.mView, MotionEvent.ACTION_BUTTON_RELEASE, displayId));
        injectMotionEvent(obtainMouseEvent(mActivity.mView, MotionEvent.ACTION_UP, displayId));
    }

    private void injectMotionEvent(MotionEvent event) {
        mInstrumentation
                .getUiAutomation()
                .injectInputEvent(event, /* sync= */ true, /* waitAnimations= */ true);
    }
    ...
    private static MotionEvent obtainMotionEvent(
            int source, View target, int action, int displayId) {
        ...
        MotionEvent event =
                MotionEvent.obtain(
                        ...
                        eventTime,
                        action,
                        xy[0] + target.getWidth() / 2,
                        xy[1] + target.getHeight() / 2,
                        /* metaState= */ 0);
