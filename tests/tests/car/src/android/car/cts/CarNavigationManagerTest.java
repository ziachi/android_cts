...
import static android.car.Car.PERMISSION_CAR_NAVIGATION_MANAGER;
...
@MediumTest
public class CarNavigationManagerTest extends AbstractCarTestCase {
    ...
    @Test
    @EnsureHasPermission(PERMISSION_CAR_NAVIGATION_MANAGER)
    ...
    public void testSendEvent() throws Exception {
        if (mCarNavigationManager == null) {
            Log.w(
                    ...
                    "Unable to run the test: "
                            + "car navigation manager was not created successfully.");
            return;
        }
        ...
        mCarAppFocusManager.addFocusListener(
                new CarAppFocusManager.OnAppFocusChangedListener() {
                    @Override
        ...
        mCarAppFocusManager.requestAppFocus(
                CarAppFocusManager.APP_FOCUS_TYPE_NAVIGATION, ownershipCallback);
        assertThat(
                        mCarAppFocusManager.isOwningFocus(
                                ownershipCallback, CarAppFocusManager.APP_FOCUS_TYPE_NAVIGATION))
                .isTrue();
