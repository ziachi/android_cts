...
public class MediaProjectionSDK34Test {
    ...
    public void testCreateVirtualDisplay_withoutCallback_throwsException() throws Exception {
        mMediaProjectionRule.startMediaProjection(/* skipDefaultCallback */ true);

        // Try to start capture with no callback registered.
        assertThrows(IllegalStateException.class, mMediaProjectionRule::createVirtualDisplay);

        mMediaProjectionRule.registerCallback(new MediaProjection.Callback() {});

        // Now starting capture should succeed.
        VirtualDisplay virtualDisplay = mMediaProjectionRule.createVirtualDisplay();
        assertNotNull(
                "VirtualDisplay should be a non-null object if capture started " + "successfully",
                virtualDisplay);
    }
    ...
    public void testCreateVirtualDisplay_reusedResultData_throwsException() throws Exception {
        ...
        MediaProjectionManager projectionManager = mMediaProjectionRule.getMediaProjectionManager();
        ...
        anotherProjection.registerCallback(
                new MediaProjection.Callback() {}, new Handler(Looper.getMainLooper()));
        // There should be an exception thrown when trying to start capture on the second
        // projection.
        assertThrows(
                SecurityException.class,
                () -> mMediaProjectionRule.createVirtualDisplay(anotherProjection));
    }
    ...
    public void testCreateVirtualDisplay_reusedMediaProjection_throwsException() throws Exception {
        ...
        // exception.
        assertThrows(SecurityException.class, () -> mMediaProjectionRule.createVirtualDisplay());
    }
}
