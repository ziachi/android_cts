...
class AppOpEventCollectionTest {
    ...
    fun ensureCorrectOpStr() {
        appOpsManager.noteOp(OPSTR_RESERVED_FOR_TESTING, myUid, myPackage, null, null)
        val opEntry = getOpEntry(myUid, myPackage, OPSTR_RESERVED_FOR_TESTING)!!
        assertThat(opEntry.opStr).isEqualTo(OPSTR_RESERVED_FOR_TESTING)
    }
    ...
    fun switchUidStateWhileOpsAreRunning() {
        ...
        appOpsManager.startOp(OPSTR_RESERVED_FOR_TESTING, myUid, myPackage, null, null)
        appOpsManager.startOp(OPSTR_RESERVED_FOR_TESTING, myUid, myPackage, null, null)

        val beforeUidChange = System.currentTimeMillis()
        sleep(1)

        assertThat(getOpEntry(myUid, myPackage, OPSTR_RESERVED_FOR_TESTING)!!
        ...
        try {
            ...
            appOpsManager.finishOp(OPSTR_RESERVED_FOR_TESTING, myUid, myPackage, null)
            appOpsManager.finishOp(OPSTR_RESERVED_FOR_TESTING, myUid, myPackage, null)
        }
    }
    ...
    fun noteWithAttributionAndCheckOpEntries() {
        val before = System.currentTimeMillis()
        appOpsManager.noteOp(OPSTR_RESERVED_FOR_TESTING, myUid, myPackage, TEST_ATTRIBUTION_TAG, null)
        ...
        assertThat(
            runWithShellPermissionIdentity {
                appOpsManager.noteOp(
                    OPSTR_RESERVED_FOR_TESTING, uid, BACKGROUND_PACKAGE, TEST_ATTRIBUTION_TAG, null
                )
            }
        ).isEqualTo(AppOpsManager.MODE_ALLOWED)
        val after = System.currentTimeMillis()

        val opEntry = getOpEntry(uid, BACKGROUND_PACKAGE, OPSTR_RESERVED_FOR_TESTING)!!
        val attributionOpEntry = opEntry.attributedOpEntries[TEST_ATTRIBUTION_TAG]!!
    ...
    fun noteSelfAndTrustedAccessAndCheckOpEntries() {
        ...
        runWithShellPermissionIdentity {
            appOpsManager.noteProxyOp(OPSTR_RESERVED_FOR_TESTING, myPackage, myUid, null, null)
        ...
        val opEntry = getOpEntry(shellUid, SHELL_PACKAGE_NAME, OPSTR_RESERVED_FOR_TESTING)!!
    ...
    fun noteForTwoAttributionsCheckOpEntries() {
        val before = System.currentTimeMillis()
        appOpsManager.noteOp(OPSTR_RESERVED_FOR_TESTING, myUid, myPackage, firstTag, null)
        ...
        // self note
        appOpsManager.noteOp(OPSTR_RESERVED_FOR_TESTING, myUid, myPackage, secondTag, null)
        val after = System.currentTimeMillis()
    ...
    fun noteFromTwoProxiesAndVerifyProxyInfo() {
        ...
        runWithShellPermissionIdentity {
            context.createAttributionContext(firstTag)
                ...
                .noteProxyOp(OPSTR_RESERVED_FOR_TESTING, otherPkg, otherUid, null, null)
        }
        ...
        val opEntry = getOpEntry(otherUid, otherPkg, OPSTR_RESERVED_FOR_TESTING)!!
        val attributionOpEntry = opEntry.attributedOpEntries[null]!!
    ...
    fun startStopTrustedProxyVerifyRunningAndTime() {
        ...
        runWithShellPermissionIdentity {
            firstAttrManager = context.createAttributionContext(firstTag)!!
                .getSystemService(AppOpsManager::class.java)!!
            val start = firstAttrManager.startProxyOpNoThrow(OPSTR_RESERVED_FOR_TESTING, otherUid, otherPkg,
                    null, null)
            assertThat(start).isEqualTo(MODE_ALLOWED)
            sleep(1)
        }

        with(getOpEntry(otherUid, otherPkg, OPSTR_RESERVED_FOR_TESTING)!!) {
        ...
        with(getOpEntry(shellUid, SHELL_PACKAGE_NAME, OPSTR_RESERVED_FOR_TESTING)!!) {
        ...
        runWithShellPermissionIdentity {
            firstAttrManager.finishProxyOp(OPSTR_RESERVED_FOR_TESTING, otherUid, otherPkg, null)
        }
        sleep(1)
    ...
    fun startStopUntrustedProxyVerifyRunningAndTime() {
        ...
        val secondAttrManager = context.createAttributionContext(secondTag)!!
            .getSystemService(AppOpsManager::class.java)!!
        secondAttrManager.startProxyOpNoThrow(OPSTR_RESERVED_FOR_TESTING, otherUid, otherPkg, null, null)
        with(getOpEntry(otherUid, otherPkg, OPSTR_RESERVED_FOR_TESTING)!!) {
        ...
        with(getOpEntry(myUid, myPackage, OPSTR_RESERVED_FOR_TESTING)!!) {
            assertThat(attributedOpEntries[secondTag]!!.isRunning).isTrue()
            assertThat(attributedOpEntries[secondTag]!!
                .getLastProxyInfo(OP_FLAGS_ALL)).isNull()
        }

        secondAttrManager.finishProxyOp(OPSTR_RESERVED_FOR_TESTING, otherUid, otherPkg, null)
        sleep(1)
    ...
    fun startProxyOpThrowsSecurityException() {
        ...
        try {
            runWithShellPermissionIdentity {
                firstAttrManager = context.createAttributionContext(firstTag)!!
                        .getSystemService(AppOpsManager::class.java)!!
                val start = firstAttrManager.startProxyOp(OPSTR_RESERVED_FOR_TESTING, Process.INVALID_UID,
                        otherPkg, null, null)
            }
    ...
    fun startStopMultipleOpsAndVerifyIsRunning() {
        ...
        appOpsManager.startOp(OPSTR_RESERVED_FOR_TESTING, myUid, myPackage, TEST_ATTRIBUTION_TAG, null)
        ...
        appOpsManager.finishOp(OPSTR_RESERVED_FOR_TESTING, myUid, myPackage, TEST_ATTRIBUTION_TAG)
