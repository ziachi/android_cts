...
package android.security.cts.BUG_277207798_provider;
...
public class ImageProvider extends ContentProvider {
    ...
    private static int getChooserUid(Context context) {
        ...
        try {
            return packageManager.getPackageUid(
    ...
    public boolean onCreate() {
        mChooserAppId = getAppId(getChooserUid(getContext()));
        return true;
    }
    ...
    private void maybeRecordUriAccess(Uri uri) {
        ...
        if (!myUserHandle().equals(caller)) {
            int callingAppId = getAppId(getCallingUid());
            if (callingAppId != mChooserAppId) {
                Log.w(
                        TAG,
                        "Ignoring cross-user access by package " + getCallingPackage()
                            + " (appId=" + callingAppId + ")");
                return;
            }
            accessedUris.add(
