...
package android.security.cts.BUG_277207798_test;
...
public class DeviceTest {
    ...
    private static final String PROVIDER_AUTHORITY = "android.security.cts.BUG_277207798_provider";
    ...
    @Test
    public void testSetUnownedUriIconAsTarget() {
        openAndCloseSharesheet(createTargetIntent(), mTargetImageUri);

        verifyNoContentProviderAccess();
    }

    private void openAndCloseSharesheet(Intent target, Uri image) {
        Instrumentation instrumentation = getInstrumentation();
        UiDevice device = UiDevice.getInstance(instrumentation);
        Context context = instrumentation.getTargetContext();
        Intent chooserIntent = Intent.createChooser(target, null);
        chooserIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        ChooserTarget customTarget = new ChooserTarget(
                "Test Target",
                Icon.createWithContentUri(image),
                0.99f,
                new ComponentName(context.getPackageName(), EmptyActivity.class.getName()),
                new Bundle());
        chooserIntent.putExtra(Intent.EXTRA_CHOOSER_TARGETS, new ChooserTarget[] { customTarget });
    ...
    private static Intent createTargetIntent() {
        ...
        intent.setType(EmptyActivity.MIME_TYPE);
        return intent;
    }
