...
public class CVE_2024_23704 extends NonRootSecurityTestCase {

    @AsbSecurityTest(cveBugId = 299931761)
    @Test
    public void testPocCVE_2024_23704() {
        try {
            // Install test-app
            installPackage("CVE-2024-23704.apk", "-t");

            // Set the 'PocAdminReceiver' as device-owner using device policy manager
            final String testPkg = "android.security.cts.CVE_2024_23704";
            try (AutoCloseable withPocAdminReceiverAsDeviceOwner =
                    withPocAdminReceiverAsDeviceOwner(testPkg)) {
                // Run DeviceTest
                runDeviceTests(
                        new DeviceTestRunOptions(testPkg)
                                .setTestClassName(testPkg + ".DeviceTest")
                                .setTestMethodName("testCVE_2024_23704")
    ...
    private AutoCloseable withPocAdminReceiverAsDeviceOwner(String testPackage) throws Exception {
        // Set the 'PocAdminReceiver' as device-owner using device policy manager
