...
final class TetheringManagerWrapper extends ServiceManagerWrapper<TetheringManager> {

    private static final String TAG = TetheringManagerWrapper.class.getSimpleName();

    private static final HashMap<Context, TetheringManager> sSpies = new HashMap<>();

    @Override
    TetheringManager getWrapper(Context context, TetheringManager manager, Answer<?> answer) {
        int userId = context.getUserId();
        TetheringManager spy = sSpies.get(context);
        if (spy != null) {
        ...
        String spyString = "TetheringManagerWrapper#" + System.identityHashCode(spy);
        ...
        try {
            ...
            doAnswer(answer).when(spy).startTethering(any(), any(), any());
        } catch (Exception e) {
